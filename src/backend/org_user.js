...
async function addUserToOrganization(UserData) {
    // User data for identifying the user whose role needs to be changed

    // let UserData = {
    //   email: "parthm@nichetech.in",
    //   firstname: "Parth",
    //   lastname: "Makwana"
    // };
    ...
    try {
      ...
      if (!users || users.length === 0) {
        ...
        let { data: org_users, error } = await supabase
          .from('org_users')
          .select('*')
...
async function modifyUserOfOrganization(UserData) {
  // Validate inputs
...
async function removeUserFromOrganization(id) {
  ...
  async function getOrgUserRole(user_id,org_id) {
    // Validate the input
